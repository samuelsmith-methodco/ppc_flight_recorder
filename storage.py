...
def _normalize_number(v: Any) -> Optional[Any]:
    """For numeric columns: return None for None, NaN, or inf so Snowflake gets NULL (avoids 'invalid identifier NAN')."""
    if v is None:
        return None
    try:
        f = float(v)
...
def _target_roas_diff_equal(old_val: Any, new_val: Any) -> bool:
    """True if old/new are equal when compared as target_roas (float with 4 decimals)."""
    try:
        o_f = float(old_val)
        n_f = float(new_val)
    except (TypeError, ValueError):
    ...
    if math.isnan(o_f) or math.isnan(n_f):
        return False
    return round(o_f, 4) == round(n_f, 4)
...
def insert_control_diff_daily(snapshot_date: date, customer_id: str, diff_rows: List[Dict[str, Any]], conn: Optional[Any] = None) -> int:
    ...
    # Drop target_roas rows where old/new are equal at 4 decimals (match state table precision)
    params_list = []
    for r in diff_rows:
        name = _safe_str(r.get("changed_metric_name"), 128)
        old_val = _normalize_diff_value(r.get("old_value"), 65535)
        new_val = _normalize_diff_value(r.get("new_value"), 65535)
        if name == "target_roas" and _target_roas_diff_equal(
    ...
    def do(conn):
        ...
        execute_many(conn, insert_sql, params_list)
        conn.commit()
        logger.info("ppc_flight_recorder: inserted %s control_state diff rows for customer_id=%s @ %s", len(params_list), customer_id, date_str)
        ...
        params_list = [
            {
                ...
                "bid_modifier": _normalize_number(r.get("bid_modifier")),
            }
            for r in rows
        ]
        execute_many(conn, insert_sql, params_list)
...
def upsert_audience_targeting_snapshot_daily(
    ...
    def do(conn):
        ...
        for i, r in enumerate(rows):
            ...
            params[prefix + "audience_size"] = _normalize_number(r.get("audience_size"))
            params[prefix + "status"] = _safe_str(r.get("status"), 32)
            params[prefix + "bid_modifier"] = _normalize_number(r.get("bid_modifier"))
...
def insert_audience_targeting_diff_daily(snapshot_date: date, customer_id: str, rows: List[Dict[str, Any]], conn: Optional[Any] = None) -> int:
    if not rows:
        return 0
    # Build params only for rows where old_value and new_value are not both null and are different (real change)
    date_str = snapshot_date.isoformat()
    params_list = []
    for r in rows:
        old_val = _normalize_diff_value(r.get("old_value"), 65535)
        ...
        params_list.append({
            ...
            "old_value": old_val,
            "new_value": new_val,
            "old_size": _normalize_number(r.get("old_size")),
            "new_size": _normalize_number(r.get("new_size")),
        })
    if not params_list:
        return 0
    ...
    def do(conn):
        ...
        execute_many(conn, insert_sql, params_list)
        conn.commit()
        logger.info("ppc_flight_recorder: inserted %s audience_targeting_diff rows for customer_id=%s @ %s", len(params_list), customer_id, date_str)
