...
def compute_control_state_diffs(current: List[Dict[str, Any]], prior: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for cur in current:
        ...
        for field in CONTROL_STATE_METRIC_FIELDS:
            ...
            # target_roas is stored in control_state as float(4); compare at 4 decimals to avoid daily diff noise
            if field == "target_roas":
                try:
                    o_f, n_f = float(ov), float(nv)
                    if not (math.isnan(o_f) or math.isnan(n_f)) and round(o_f, 4) == round(n_f, 4):
                        continue
                except (TypeError, ValueError):
                    pass
            ...
            if field == "target_roas":
                try:
                    if old_str is not None and not _diff_value_empty(ov):
                        old_str = f"{round(float(ov), 4):.4f}"
                    if new_str is not None and not _diff_value_empty(nv):
                        new_str = f"{round(float(nv), 4):.4f}"
                except (TypeError, ValueError):
                    pass
